...
static int my_kern_thread(void *arg) {
    cprintf(" my_kernel_thread, pid = %d, name = %s\n", current->pid, get_proc_name(current));
	schedule();
    cprintf(" my_kernel_thread, pid = %d, name = %s , arg  %s \n", current->pid, get_proc_name(current), (const char *)arg);
	schedule();
    cprintf(" my_kernel_thread, pid = %d, name = %s ,  en.., Bye, Bye. :)\n",current->pid, get_proc_name(current));
    return 0;
}
